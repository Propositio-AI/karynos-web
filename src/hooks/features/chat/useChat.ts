...
export const useChat = (conversationId: string) => {
    ...
    const fetchChatData = useCallback(async () => {
        setLoading(true);
        try {
            const data = await api.getConversationDetailsApiV1ChatConversationConversationIdGet(conversationId);
    ...
    return { messages, title, loading, isSending, sendMessage, deleteMessage, refresh: fetchChatData };
};
