...
import { getCurrentAuthState, subscribeAuthChanged } from "@/lib/auth/session";
...
const isOnboardingAnswered = async (allowLocalStatus: boolean) => {
	if (allowLocalStatus && hasLocalOnboardingAnswer(ONBOARDING_VERSION)) {
		return true;
	}

	const history = await api.getAnswerHistoryApiV1OnboardingAnswersHistoryGet();

	return history.some((answer) => answer.question_version === ONBOARDING_VERSION);
};
...
export const AuthGate = ({ children }: { children: React.ReactNode }) => {
	...
	useEffect(() => {
		...
		const checkAuth = async () => {
			...
			if (state.isAuthenticated && !isPublic && pathname !== ONBOARDING_PATH) {
				try {
					const answered = await isOnboardingAnswered(state.mode === "mock");
					if (cancelled) return;
