...
export const InitQuestionsPageContent = () => {
	...
	const handleOptionSelect = (question: InitQuestionResponse, optionId: string) => {
	...
	const isAnsweredAll = orderedQuestions.length > 0 && answeredCount === orderedQuestions.length;
		...
		<main className="min-h-screen bg-canvas px-4 pb-48 pt-6 sm:px-6 sm:pt-10">
			<div className="mx-auto w-full max-w-2xl">
				<section className="mb-6 rounded-(--radius-md) border border-line bg-surface p-5 sm:p-7">
					<p className="mb-2 text-sm font-semibold text-brand-700">First match setup</p>
					<h1 className="text-2xl text-ink sm:text-3xl">初期診断</h1>
								...
								{(question.options ?? []).map((option) => {
									...
									return (
										<button
											...
											onClick={() =>
												handleOptionSelect(question, option.option_id)
											}
