...
export const SwipeResultMessage = ({ swipeDirection, onReset }: SwipeResultMessageProps) => {
	...
	return (
		...
		>
			...
			<p className={`mb-8 text-xl font-semibold ${isGood ? "text-brand-700" : "text-muted"}`}>
				{isGood ? "いいねしました" : "パスしました"}
			</p>
			<BaseButton color="white" className="mx-auto w-full max-w-xs" onClick={onReset}>
				次へ
			</BaseButton>
		</motion.div>
	);
