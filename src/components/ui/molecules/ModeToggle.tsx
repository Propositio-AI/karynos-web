    ...
    return (
        <div className="flex border border-zinc-200 rounded-lg">
            <button
                ...
                className={`px-5 py-2 text-sm font-medium rounded-l-md ${selected === 'single' ? 'bg-blue-500 text-white' : 'bg-zinc-50 text-zinc-500'}`}
            >
                単一
            </button>
            <button
                onClick={() => handleToggle('bulk')}
                className={`px-5 py-2 text-sm font-medium rounded-r-md ${selected === 'bulk' ? 'bg-blue-500 text-white' : 'bg-zinc-50 text-zinc-500'}`}
            >
                一括
            </button>
        </div>
