            ...
            <VerticalStackContainer space={8}>
                ...
                <div className="flex justify-end">
                    <Link href="/mentor/dreamer/group/new">
                        <IconButton icon={faPlus} color="blue" className="font-semibold !rounded-full px-4">
                            グループ追加
                        </IconButton>
                    </Link>
                </div>
                ...
                <GridContainer minWidth={300} className="gap-4">
                    {groups.map(group => (
                        <GroupCard key={group.id} id={group.id} name={group.name} memberCount={group.memberCount} />
                    ))}
                </GridContainer>
